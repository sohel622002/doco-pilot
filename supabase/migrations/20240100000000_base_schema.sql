-- ============================================================
-- Base schema. Mirrors server/supabase-schema.sql — kept as its own
-- migration (earliest timestamp) so `supabase start` / `supabase db reset`
-- can apply the full schema to a local stack in one go, since the later
-- migrations in this folder (google_oauth, server_alerts, etc.) assume
-- these tables already exist. If you change server/supabase-schema.sql,
-- mirror the change here too, or add a new incremental migration instead —
